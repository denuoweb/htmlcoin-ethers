...
exports.HtmlcoinWallet = exports.IdempotencyError = exports.defaultPath = exports.SLIP_BIP44_PATH = exports.HTMLCOIN_BIP44_PATH = void 0;
...
class HtmlcoinWallet extends IntermediateWallet_1.IntermediateWallet {
    constructor(privateKey, provider, opts) {
        ...
        if (provider && !provider.getUtxos) {
            // throw new Error("HtmlcoinWallet provider requires getUtxos method: see HtmlcoinProvider")
        }
        super(privateKey, provider);
        this.htmlcoinProvider = provider;
        this.opts = opts || {};
    }
    ...
    async signTransaction(transaction) {
        ...
        if (this.provider.isClientVersionGreaterThanEqualTo) {
            ...
            throw new Error("Must use HtmlcoinProvider");
        }
        const augustFirst2022 = 1659330000000;
        const mayThirtith2022 = 1653886800000;
        const now = new Date().getTime();
        const requireFixedJanus = now > augustFirst2022;
        const message = "You are using an outdated version of Janus that has a bug that htmlcoin-ethers-wrapper works around, " +
        ...
        if (transactionType === global_vars_1.GLOBAL_VARS.DEPLOY_ERROR) {
            return logger.throwError("You cannot send HTMLCOIN while deploying a contract. Try deploying again without a value.", utils_1.Logger.errors.NOT_IMPLEMENTED, {
    ...
    async getUtxos(from, neededAmount, types = ["p2pk", "p2pkh"]) {
        ...
        if (!this.htmlcoinProvider) {
            throw new Error("No provider defined");
        }
        const result = await this.do("htmlcoin_qetUTXOs", params);
        if (result) {
    ...
    static fromPrivateKey(privateKey) {
        return new HtmlcoinWallet(privateKey);
    }
    /**
    ...
    static fromEncryptedJsonSync(json, password) {
        return new HtmlcoinWallet(json_wallets_1.decryptJsonWalletSync(json, password));
    }
    /**
    ...
    static fromMnemonic(mnemonic, path, wordlist) {
        ...
        return new HtmlcoinWallet(utils_2.configureHtmlcoinAddressGeneration(hdnode));
    }
}
exports.HtmlcoinWallet = HtmlcoinWallet;
