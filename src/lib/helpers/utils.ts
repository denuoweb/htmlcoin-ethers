...
import { HtmlcoinTransactionRequest } from './IntermediateWallet';
...
export async function addVins(
    ...
    const neededAmountBN = BigNumberEthers.from(new BigNumber(htmlcoinToSatoshi(neededAmount)).toString());
...
function satoshiToHtmlcoin(inSatoshi?: BigNumberish): string {
    return shiftBy(inSatoshi || 0, -8);
}

function htmlcoinToSatoshi(inHtmlcoin?: BigNumberish): string {
    return shiftBy(inHtmlcoin || 0, 8);
}

function checkLostPrecisionInGasPrice(gasPrice: number) {
    const roundedGasPrice = new BigNumber(new BigNumber(satoshiToHtmlcoin(gasPrice)).toFixed(8)).toNumber();
    const originalGasPrice = new BigNumber(new BigNumber(satoshiToHtmlcoin(gasPrice)).toFixed()).toNumber();
...
export function configureHtmlcoinAddressGeneration(hdnode: HDNode): HDNode {
...
export function checkTransactionType(tx: TransactionRequest): CheckTransactionType {
    if (!!tx.to === false && (!!tx.value === false || BigNumberEthers.from(tx.value).toNumber() === 0) && !!tx.data === true) {
        const needed = new BigNumber(satoshiToHtmlcoin(tx.gasPrice)).times(BigNumberEthers.from(tx.gasLimit).toNumber()).toFixed(8).toString()
        ...
        const needed = !!tx.value === true ? 
            new BigNumber(
                new BigNumber(satoshiToHtmlcoin(tx.gasPrice)).toFixed(8))
                    .times(BigNumberEthers.from(tx.gasLimit).toNumber())
                    .plus(satoshiToHtmlcoin(tx.value)).toFixed(8) :
            new BigNumber(new BigNumber(satoshiToHtmlcoin(tx.gasPrice)).toFixed(8))
        ...
        const gas = new BigNumber(satoshiToHtmlcoin(tx.gasPrice)).times(BigNumberEthers.from(tx.gasLimit).toNumber());
        const needed = new BigNumber(satoshiToHtmlcoin(tx.value)).plus(gas).toFixed(8);
...
export async function serializeTransactionWith(
    ...
    let htmlcoinTx: Tx = { version: 2, locktime: 0, vins: [], vouts: [] };
    ...
    if (transactionType === GLOBAL_VARS.CONTRACT_CREATION) {
        ...
        const contractVoutValue = !!tx.value === true ?
            new BigNumber(satoshiToHtmlcoin(tx.value)).toNumber() :
            new BigNumber(BigNumberEthers.from("0x0").toNumber() + `e-8`).toFixed(8);
    ...
    for (let i = 0; i < htmlcoinTx.vins.length; i++) {
        if (vinTypes[i].toLowerCase() === "p2pk")  {
            updatedVins.push({ ...htmlcoinTx.vins[i], ['scriptSig']: p2pkScriptSig(await signp2pkhWith(htmlcoinTx, i, signer)) })
        } else {
...
export const messagePrefix = "\x15Htmlcoin Signed Message:\n";
