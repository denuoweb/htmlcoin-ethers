...
async function addVins(outputs, spendableUtxos, neededAmount, needChange, gasPriceString, hash160PubKey, publicKey) {
    ...
    const neededAmountBN = ethers_1.BigNumber.from(new bignumber_js_1.BigNumber(htmlcoinToSatoshi(neededAmount)).toString());
...
function satoshiToHtmlcoin(inSatoshi) {
    return shiftBy(inSatoshi || 0, -8);
}
function htmlcoinToSatoshi(inHtmlcoin) {
    return shiftBy(inHtmlcoin || 0, 8);
}
function checkLostPrecisionInGasPrice(gasPrice) {
    const roundedGasPrice = new bignumber_js_1.BigNumber(new bignumber_js_1.BigNumber(satoshiToHtmlcoin(gasPrice)).toFixed(8)).toNumber();
    const originalGasPrice = new bignumber_js_1.BigNumber(new bignumber_js_1.BigNumber(satoshiToHtmlcoin(gasPrice)).toFixed()).toNumber();
...
function configureHtmlcoinAddressGeneration(hdnode) {
    ...
    // @ts-ignore
    properties_1.defineReadOnly(hdnode, "htmlcoinAddress", computeAddress(hdnode.publicKey, true));
    return hdnode;
}
exports.configureHtmlcoinAddressGeneration = configureHtmlcoinAddressGeneration;
function checkTransactionType(tx) {
    if (!!tx.to === false && (!!tx.value === false || ethers_1.BigNumber.from(tx.value).toNumber() === 0) && !!tx.data === true) {
        const needed = new bignumber_js_1.BigNumber(satoshiToHtmlcoin(tx.gasPrice)).times(ethers_1.BigNumber.from(tx.gasLimit).toNumber()).toFixed(8).toString();
        ...
        const needed = !!tx.value === true ?
            new bignumber_js_1.BigNumber(new bignumber_js_1.BigNumber(satoshiToHtmlcoin(tx.gasPrice)).toFixed(8))
                .times(ethers_1.BigNumber.from(tx.gasLimit).toNumber())
                .plus(satoshiToHtmlcoin(tx.value)).toFixed(8) :
            new bignumber_js_1.BigNumber(new bignumber_js_1.BigNumber(satoshiToHtmlcoin(tx.gasPrice)).toFixed(8))
        ...
        const gas = new bignumber_js_1.BigNumber(satoshiToHtmlcoin(tx.gasPrice)).times(ethers_1.BigNumber.from(tx.gasLimit).toNumber());
        const needed = new bignumber_js_1.BigNumber(satoshiToHtmlcoin(tx.value)).plus(gas).toFixed(8);
...
async function serializeTransactionWith(utxos, fetchUtxos, neededAmount, tx, transactionType, signer, publicKey, opts) {
    ...
    if (transactionType === global_vars_1.GLOBAL_VARS.CONTRACT_CREATION) {
        ...
        htmlcoinTx.vouts.push(contractCallVout);
    }
        ...
        if (needChange) {
            ...
            neededAmount = satoshiToHtmlcoin(neededAmountBN);
        }
        ...
        // user requested sending HTMLCOIN with OP_CREATE which will result in the HTMLCOIN being lost
        throw new Error("Cannot send HTMLCOIN to contract when deploying a contract");
    }
    else {
    ...
    try {
        ...
        const neededAmountMinusGas = satoshiToHtmlcoin(neededAmountMinusGasBN);
        // @ts-ignore
    ...
    if (transactionType === global_vars_1.GLOBAL_VARS.P2PKH) {
        ...
        if (value != 0) {
            ...
            htmlcoinTx.vouts.push(p2pkhVout);
        }
    }
    // add change if needed
    if (changeAmount) {
        htmlcoinTx.vouts.push({
            // @ts-ignore
    ...
    for (let i = 0; i < htmlcoinTx.vins.length; i++) {
        if (vinTypes[i].toLowerCase() === "p2pk") {
            updatedVins.push(Object.assign(Object.assign({}, htmlcoinTx.vins[i]), { ['scriptSig']: p2pkScriptSig(await signp2pkhWith(htmlcoinTx, i, signer)) }));
        }
        else {
    ...
    return txToBuffer(htmlcoinTx).toString('hex');
}
...
exports.messagePrefix = "\x15Htmlcoin Signed Message:\n";
...
function splitSignature(signature, vrs) {
    ...
    if (bytes_1.isBytesLike(signature)) {
        ...
        if (buffer.length >= 65) {
            ...
            if (vrs) {
                throw new Error("EIP-2098 Compact Signature Representation unsupported when decoding signature in Htmlcoin format (VRS vs RSV)");
            }
