...
import { HtmlcoinWallet, HTMLCOIN_BIP44_PATH } from "./HtmlcoinWallet";
import { parseSignedTransaction } from "./helpers/utils";
export class HtmlcoinTestProvider extends HtmlcoinProvider {
    constructor(options) {
        super(options ?
        ...
        if (!options.accounts || options.accounts.length === 0) {
            if (options.mnemonic) {
                this.wallets.push(HtmlcoinWallet.fromMnemonic(options.mnemonic, options.hd_path || HTMLCOIN_BIP44_PATH).connect(this));
                for (let j = 1; j < 5; j++) {
                    this.wallets.push(HtmlcoinWallet.fromMnemonic(options.mnemonic, (options.hd_path || HTMLCOIN_BIP44_PATH) + "/" + j).connect(this));
                }
            }
            else {
                options.accounts = [
        ...
        if (options.accounts && options.accounts.length !== 0) {
            for (let i = 0; i < options.accounts.length; i++) {
                // @ts-ignore
                this.wallets.push(new HtmlcoinWallet(options.accounts[i].privateKey).connect(this));
            }
        }
    }
    ...
    createEmptyWallet() {
        return HtmlcoinWallet.createRandom().connect(this);
    }
    clearCallHistory() {
        this._callHistory = [];
    }
