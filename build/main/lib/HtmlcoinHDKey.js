"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.HtmlcoinHDKey = void 0;
const hdnode_1 = require("@ethersproject/hdnode");
const utils_1 = require("./helpers/utils");
const HtmlcoinWallet_1 = require("./HtmlcoinWallet");
class HtmlcoinHDKey {
    constructor(hdkey) {
        this._hdkey = hdkey;
        utils_1.configureHtmlcoinAddressGeneration(hdkey);
    }
    static fromMasterSeed(seedBuffer) {
        const hdnode = utils_1.configureHtmlcoinAddressGeneration(hdnode_1.HDNode.fromSeed("0x" + seedBuffer.toString('hex')));
        return new HtmlcoinHDKey(hdnode);
    }
    static fromExtendedKey(base58Key) {
        const hdnode = utils_1.configureHtmlcoinAddressGeneration(hdnode_1.HDNode.fromExtendedKey("0x" + base58Key));
        return new HtmlcoinHDKey(hdnode);
    }
    privateExtendedKey() {
        if (!this._hdkey.privateKey) {
            throw new Error('This is a public key only wallet');
        }
        return Buffer.from(this._hdkey.extendedKey);
    }
    publicExtendedKey() {
        return Buffer.from(this._hdkey.neuter().extendedKey);
    }
    derivePath(path) {
        return new HtmlcoinHDKey(utils_1.configureHtmlcoinAddressGeneration(hdnode_1.HDNode.fromExtendedKey(this._hdkey.extendedKey).derivePath(path)));
    }
    deriveChild(index) {
        return new HtmlcoinHDKey(
        // @ts-ignore
        utils_1.configureHtmlcoinAddressGeneration(hdnode_1.HDNode.fromExtendedKey(this._hdkey.extendedKey)._derive(index)));
    }
    getWallet() {
        return new HtmlcoinWallet_1.HtmlcoinWallet(utils_1.configureHtmlcoinAddressGeneration(hdnode_1.HDNode.fromExtendedKey(this._hdkey.extendedKey)));
    }
}
exports.HtmlcoinHDKey = HtmlcoinHDKey;
//# sourceMappingURL=data:application/json;base64,