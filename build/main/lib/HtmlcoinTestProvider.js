...
exports.HtmlcoinTestProvider = void 0;
const ethers_1 = require("ethers");
const HtmlcoinProvider_1 = require("./HtmlcoinProvider");
const HtmlcoinWallet_1 = require("./HtmlcoinWallet");
const utils_1 = require("./helpers/utils");
class HtmlcoinTestProvider extends HtmlcoinProvider_1.HtmlcoinProvider {
    constructor(options) {
        super(options ?
        ...
        if (!options.accounts || options.accounts.length === 0) {
            if (options.mnemonic) {
                this.wallets.push(HtmlcoinWallet_1.HtmlcoinWallet.fromMnemonic(options.mnemonic, options.hd_path || HtmlcoinWallet_1.HTMLCOIN_BIP44_PATH).connect(this));
                for (let j = 1; j < 5; j++) {
                    this.wallets.push(HtmlcoinWallet_1.HtmlcoinWallet.fromMnemonic(options.mnemonic, (options.hd_path || HtmlcoinWallet_1.HTMLCOIN_BIP44_PATH) + "/" + j).connect(this));
                }
            }
            else {
                options.accounts = [
        ...
        if (options.accounts && options.accounts.length !== 0) {
            for (let i = 0; i < options.accounts.length; i++) {
                // @ts-ignore
                this.wallets.push(new HtmlcoinWallet_1.HtmlcoinWallet(options.accounts[i].privateKey).connect(this));
            }
        }
    }
    ...
    createEmptyWallet() {
        return HtmlcoinWallet_1.HtmlcoinWallet.createRandom().connect(this);
    }
    clearCallHistory() {
        this._callHistory = [];
    }
...
exports.HtmlcoinTestProvider = HtmlcoinTestProvider;
